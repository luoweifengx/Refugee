...
public final class NurseService {
	...
	public static HealResult heal(ServerPlayer player, Villager villager) {
		return heal(player, villager, false);
	}

	public static HealResult heal(ServerPlayer player, Villager villager, boolean free) {
		...
		int fee = free ? 0 : computeFee(player);
		if (fee > 0 && countEmeralds(player) < fee) {
